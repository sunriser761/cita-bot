...
def send_telegram(msg):
    ...
    for chat_id in CHAT_IDS:
        try:
            ...
            print("chat_id " + chat_id + ": " + str(r.status_code), flush=True)
...
def test_network():
    ...
    try:
        ...
        print("Network OK: " + str(r.status_code), flush=True)
        return True
    except Exception as e:
        print("Network FAILED: " + str(e), flush=True)
        return False


def open_and_select_province(driver):
    print("Step 1: Province", flush=True)
    driver.get(URL)
    time.sleep(4)
...
def select_office_and_tramite(driver):
    print("Step 2: Office and tramite", flush=True)
    wait = WebDriverWait(driver, 20)
...
def click_enter(driver):
    print("Step 3: Entrar", flush=True)
    wait = WebDriverWait(driver, 20)
    try:
        js_click(driver, wait.until(EC.presence_of_element_located((By.ID, "btnEntrar"))))
        time.sleep(3)
    except TimeoutException:
        print("btnEntrar not found", flush=True)


def fill_user_data(driver):
    print("Step 4: NIE and name", flush=True)
    wait = WebDriverWait(driver, 20)
...
def solicitar_cita(driver):
    print("Step 5: Solicitar Cita", flush=True)
    wait = WebDriverWait(driver, 20)
    try:
        js_click(driver, wait.until(EC.presence_of_element_located((By.ID, "btnEnviar"))))
        time.sleep(4)
    except TimeoutException:
        print("btnEnviar not found", flush=True)
...
def main():
    ...
    # Діагностика Chrome
    try:
        v = subprocess.check_output(["google-chrome", "--version"]).decode().strip()
        print("Chrome: " + v, flush=True)
    except Exception as e:
        print("Chrome not found: " + str(e), flush=True)

    try:
        v = subprocess.check_output(["/usr/local/bin/chromedriver", "--version"]).decode().strip()
        print("Chromedriver: " + v, flush=True)
    except Exception as e:
        print("Chromedriver not found: " + str(e), flush=True)

    # Тест мережі
    network_ok = test_network()
    if not network_ok:
        send_telegram("ERROR: Cannot reach the site - network blocked!")
        print("Exiting due to network failure", flush=True)
        return

    send_telegram("Bot started - monitoring TARJETA CONFLICTO UCRANIA")
    driver = create_driver()
    print("Driver created OK", flush=True)
    ...
    while True:
        ...
        try:
            ...
            print("Timeout: " + str(e)[:100], flush=True)
            try:
                driver.quit()
            except Exception:
                pass
        ...
        print("Next check in " + str(CHECK_INTERVAL) + "s", flush=True)
